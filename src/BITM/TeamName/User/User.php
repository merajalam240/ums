...
class User extends DB{
    ...
    public function store() {
       ...
       $dataArray= array(':firstName'=>$this->firstName,':lastName'=>$this->lastName,':email'=>$this->email,':password'=>$this->password,
        ...
        $query="INSERT INTO `ums`.`users` (`first_name`, `last_name`, `email`, `password`, `phone`, `address`,`email_verified`) 
VALUES (:firstName, :lastName, :email, :password,:phone, :address, :email_token)";
    ...
    public function change_password(){
        $query="UPDATE `ums`.`users` SET `password`=:password  WHERE `users`.`email` =:email";
    ...
    public function validTokenUpdate(){
        $query="UPDATE `ums`.`users` SET  `email_verified`='".'Yes'."' WHERE `users`.`email` ='$this->email'";
        $result=$this->conn->prepare($query);
        $result->execute();
